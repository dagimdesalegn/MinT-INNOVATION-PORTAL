...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Clear localStorage on failed login to prevent showing old/mocked user
  ...
  const handleGoogleLogin = () => {
    // Use ApiService for Google OAuth
    api.googleLogin();
  };
          ...
          <CardContent className="space-y-4">
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex justify-end">
                <Link to="/forgot-password" className="text-sm text-innovation-600 hover:text-innovation-700">
                  Forgot password?
                </Link>
              </div>
